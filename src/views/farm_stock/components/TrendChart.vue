  ...
  methods: {
    async getTrendData() {
      try {
        ...
        let res = await this.$service.getSensorListByLandCode({farmId: id, landCode: this.landCode});
        if (res && res.code === 0) {
          if (_.get(res.data, 'list.length') > 0) {
            let deviceName = _.get(res.data, `list.0.deviceName`);
            let sensorType = _.get(res.data, `list.0.sensorType`);
            ...
            if (deviceName) {
              ...
              let res2 = await this.$service.getSensor({deviceName, days});
              if (res2 && res2.code === 0) {
